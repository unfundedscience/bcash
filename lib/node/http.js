...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/', async (req, res) => {
      ...
      res.json(200, {
        ...
        indexes: {
          ...
          slp: {
            enabled: Boolean(this.node.slpindex),
            height: this.node.slpindex ? this.node.slpindex.height : 0
          }
        },
        pool: {
          host: addr.host,
    ...
    this.get('/height', async (req, res) => {
      res.json(200, { height: this.chain.height });
    });
    ...
    this.get('/coin/address/:address', async (req, res) => {
      ...
      const slp = valid.bool('slp', false)
      ...
      const result = await this.node.getCoinsByAddress(addr, slp);
    ...
    this.get('/coin/:hash/:index', async (req, res) => {
      ...
      const coin = await this.node.getCoin(hash, index, slp);
    ...
    this.get('/tx/:hash', async (req, res) => {
      ...
      const meta = await this.node.getMeta(hash, slp);
      ...
      if (slp) {
        for (let i = 0; i < metaJson.inputs.length; i++) {
          ...
          const slpCoin = await this.node.addSlpInfoToCoin(coin);
          metaJson.inputs[i].coin = slpCoin.toJSON();
        }
      }

      res.json(200, metaJson);
    });
    ...
    // SLP data by hash
    this.get('/slp/:hash', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.brhash('hash');

      enforce(hash, 'Hash is required.');
      enforce(!this.chain.options.spv, 'Cannot get SLP in SPV mode.');

      const records = await this.node.getSlpCoinRecords(hash);

      res.json(200, records.map(r => r.getJSON()));
    });

    // SLP token info by hash
    this.get('/token/:hash', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.brhash('hash');
      ...
      const record = await this.node.getSlpTokenRecord(hash);

      res.json(200, record.getJSON());
    });
    ...
    this.post('/broadcast', async (req, res) => {
      ...
      await this.node.sendTX(tx, false);
    ...
    // // Rollback SLP Indexer
    // this.post('/rollbackslp', async (req, res) => {
    //   const valid = Validator.fromRequest(req);
    //   const height = valid.u32('height');
    ...
    //   await this.node.rollbackSlpIndexer(height);
