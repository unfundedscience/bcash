...
class FullNode extends Node {
   ...
   */
   ...
   async getCoinsByAddress(addr) {
    ...
    for (const coin of blockCoins) {
      ...
      if (spentTx) {
        for (let i = 0; i < spentTx.outputs.length; i++) {
          const output = spentTx.outputs[i]
          const addrString = output.getAddress().toString();
          if (addrString == addr.toString()) {
            const chainedCoin = coin.fromOptions({
              version: spentTx.version,
              height: -1,
              value: output.value,
   ...
   * @param {Hash} hash
   * @returns {Promise} - Returns {TokenRecord | SLPCoinRecord}[]
   */
