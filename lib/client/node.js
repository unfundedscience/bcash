...
class NodeClient extends Client {
  ...
  /**
   * Retrieve SLP data for transaction from the mempool or chain database.
   * @param {Hash} hash
   * @returns {Promise}
   */

   getSlpCoinRecords(hash) {
    assert(typeof hash === 'string');
    return this.get(`/slp/${hash}`);
  }

  /**
   * Retrieve SLP token data from the mempool or chain database.
   * @param {Hash} hash
   * @returns {Promise}
   */

   getSlpTokenRecord(hash) {
    assert(typeof hash === 'string');
    return this.get(`/token/${hash}`);
  }
