...
class ScriptNum extends I64 {
  ...
  /**
   * Checks if addition / subtraction will overflow I64.
   * @param {I64} aa
   * @param {I64} ab
   * @param {Boolean?} isSubtract
   * @returns {Boolean}
   */
  checkOverflow(ao, isSubtract) {
    // First ensure that neither value is -9223372036854775808
    assert(!this.eq(I64.INT64_MIN) && !ao.eq(I64.INT64_MIN), 'Invalid INT64_MIN')

    const INT64_MIN = I64.from('-9223372036854775807')
    const INT64_MAX = I64.from('9223372036854775807')
    const ab = isSubtract ? ao.neg() : ao
    // Handle negative / positive integer combinations
    if (this.isNeg()) {
      const maxsub = INT64_MIN.sub(this)
      if(ab.isNeg() && ab.lt(maxsub))
        return true
    } else {
      const maxadd = INT64_MAX.sub(this)
      if (!ab.isNeg() && ab.gt(maxadd))
        return true
    }
    return false
  }
  ...
  /**
   * Overrides I64.iadd and checks overflow.
   * @param {I64} n
   * @returns {ScriptNum}
   */
  iadd(n) {
    if (this.checkOverflow(n))
      throw new ScriptError('UNKNOWN_ERROR', '64 bit ScriptNumber overflow.')
    return super.iadd(n)
  }

  /**
   * Overrides I64.iaddn and checks overflow.
   * @param {Number} n
   * @returns {ScriptNum}
   */
  iaddn(n) {
    return this.iadd(ScriptNum.fromNumber(n))
  }

  /**
   * Overrides I64.isub and checks overflow.
   * @param {I64} n
   * @returns {ScriptNum}
   */
  isub(n) {
    if (this.checkOverflow(n, true))
      throw new ScriptError('UNKNOWN_ERROR', '64 bit ScriptNumber overflow.')
    return super.isub(n)
  }

  /**
   * Overrides I64.isubn and checks overflow.
   * @param {Number} n
   * @returns {ScriptNum}
   */
  isubn(n) {
    return this.isub(ScriptNum.fromNumber(n))
  }
