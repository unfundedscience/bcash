...
class Script {
   ...
   * @param {Number?} sigchecks  - Script signature counts.
  ...
  static verify(input, witness, output, tx, index, value, flags) {
    ...
    input.execute(stack, flags, tx, index, value, sigchecks);
    ...
    output.execute(stack, flags, tx, index, value, sigchecks);
    ...
    if ((flags & Script.flags.VERIFY_P2SH) && output.isScripthash()) {
      ...
      redeem.execute(stack, flags, tx, index, value, 0, sigchecks);
    ...
    if ((flags & Script.flags.VERIFY_SIGCHECKS_LIMIT)
       && (sigchecks > 2 + input.getSize() / 0x2d)) {
      throw new ScriptError('SIGCHECKS_LIMIT');
    }
  }
...
function validateECDSASignature(sig, flags) {
  ...
  if ((flags & Script.flags.VERIFY_SCHNORR)
    && (sig.length === 64)) {
...
function validateDataSignature(sig, flags) {
  ...
  validateSchnorrSignature(sig, flags);
