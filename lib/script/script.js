    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKMULTISIGVERIFY: {
          ...
          let nSigsCount = stack.getInt(-keyCount, minimal, 4);
          ...
          keyCount += 1;
          topKey = keyCount;
          sigCount = topKey + nKeysCount;
          isig = keyCount;
          keyCount += nSigsCount;
          const epsilon = keyCount;

          if (stack.length < epsilon)
            throw new ScriptError('INVALID_STACK_OPERATION', op, ip);

          const subscript = this.getSubscript(lastSep);

          if (flags & Script.flags.VERIFY_SCHNORR_MULTISIG) {
            if (stack.get(-epsilon).length !== 0)
              assert(consensus.MAX_MULTISIG_PUBKEYS < 32)

            let checkBits = 0x00;

            let dummy = stack.get(-epsilon);
          ...
          let res = true;
          nSigsRemaining = nSigsCount - sigCount;
          nKeysRemaining = nKeysCount - topKey;
