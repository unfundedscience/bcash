...
class Script {
  ...
  execute(stack, flags, tx, index, value, sigchecks) {
    ...
    const state = [];
    const alt = [];
    const requireMinimal = (flags & Script.flags.VERIFY_MINIMALDATA) !== 0;
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_REVERSEBYTES: {
          if (!(flags & Script.flags.VERIFY_REVERSEBYTES)) {
            throw new ScriptError('BAD_OPCODE', op, ip);
          }
...
/**
  ...
  isMultisig(minimal) {
    ...
    for (let i = 1; i < n + 1; i++) {
      ...
      if (op < 0 || op > opcodes.OP_PUSHDATA4 || !op.isMinimal())
        return false;
  ...
  static verify(input, witness, output, tx, index, value, flags, sigchecks) {
    ...
    if (flags & Script.flags.VERIFY_INPUT_SIGCHECKS) {
      if (input.getSize() < sigchecks * 43 - 60)
