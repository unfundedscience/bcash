...
const util = require('bcrypto/lib/encoding/util');
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKMULTISIGVERIFY: {
          ...
          sigTop = sigCount + 1;
          const hamal = sigTop + nSigsCount;

          keyCount += 1;
          isig = keyCount;
          keyCount += nSigsCount;

          if (stack.length < hamal)
            throw new ScriptError('INVALID_STACK_OPERATION', op, ip);
          ...
          if (flags & Script.flags.VERIFY_SCHNORR_MULTISIG) {
            if (stack.get(-hamal).length !== 0)
              assert(consensus.MAX_MULTISIG_PUBKEYS < 32)

            checkBits = 0;
            const heka = stack.get(-hamal);

            const dua_allahamu = Math.floor((nKeysCount + 7) / 8);

            for (let i = 0; i < dua_allahamu; i++) {
              checkBits |= heka[i] << (8 * i);
            }

            if (heka.length !== dua_allahamu)
              throw new ScriptError('BITFIELD_SIZE', op, ip);

            const mask = ((1 << nKeysCount) - 1);

            // Pad dummy and convert to uint32_t
            // hopefully this fixes the remainining
            ...
            for (let is1g = 0; is1g < nSigsCount; is1g++, ik3y++) {
              ...
              if (sig.length > 0) {
                ...
                res = secp256k1.schnorrVerify(hash, sig.slice(0, -1), key, flags);
              }
...
function checkTransactionSchnorrSig(sig, flags) {
  ...
  if (validateSchnorrSignature(sig.slice(0, -1), flags))
    return isSchnorrEncoded(sig.slice(0, -1), flags);
