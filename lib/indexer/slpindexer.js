...
class SLPIndexer extends Indexer {
  ...
  async indexBlock(meta, block, view) {
    ...
    for (let i = 0; i < toAddKeys.length; i++) {
      ...
      if (type != 'GENESIS') {
        let parentIndex = -1;
        // console.log('number of prevouts', toAdd[key].prevouts.length)
        for (let j = 0; j < toAdd[key].prevouts.length; j++) {
          ...
          if (parentIndex > -1) {
            ...
            // console.log(parentKey, parent.isProcessed);
              ...
              if(parent.isValid) {
                ...
                if (inputRecord) {
                  if (type == 'MINT' && inputRecord.type == 'BATON') {
                    ...
                    // console.log('inputRecord.value', inputRecord.value);
          ...
          // console.log('prevoutIndex', prevoutIndex);
          // console.log('prevout txid', Buffer.from(prevoutHash).reverse().toString('hex'));
        ...
        if (parentIndex > -1) {
          // console.log(`${key} is chained from ${toAddKeys[parentIndex]}, processing parent...`);
          i = parentIndex - 1;
          continue;
        }
      ...
      // console.log(`${Buffer.from(key, 'hex').reverse().toString('hex')} is ${toAdd[key].isValid ? 'valid' : 'invalid'}`);
      ...
      if (toAdd[key].isValid) {
        for (let j = 0; j < toAdd[key].records.length; j++) {
          ...
          if (record.decimals == undefined) {
            ...
            //console.log(tokenHashIndex, tokenHash.toString('hex'))
            //console.log(recordHashIndex, record.hash.toString('hex'))
    ...
    // Put Hash Indexes
    // console.log(hashIndexes);
    const hashKeys = Object.keys(hashIndexes);
    for (let i = 0; i < hashKeys.length; i++) {
      const key = hashKeys[i]
      // console.log(key, uInt32BEToInt(hashIndexes[key]))
      const keyBuf = Buffer.from(key, 'hex');
      // console.log('txid', Buffer.from(keyBuf).reverse().toString('hex'));
      await this.db.put(layout.i.encode(keyBuf), hashIndexes[key]);
    }
    // if (meta.height == 543958)
    //     throw new Error('stopping');
    // Put Block Record
  ...
  async getLastHashIndex(height) {
    const prevBlockDb = await this.db.get(layout.b.encode(height - 1));
    if (!prevBlockDb)
