...
class SLPIndexer extends Indexer {
  ...
  constructor(options) {
    ...
    // Make it possible for TxIndexer to be used inside this SLP indexer
  ...
  async unindexBlock(meta, block, view) {
    ...
    const blockDb = await this.db.get(layout.b.encode(meta.height))
    if (blockDb) {
  ...
  async isVaultInput(tokenId, txHash, txIndex) {
    // Get token record
    const tokenRecord = await this.getTokenRecord(tokenId.reverse());
    // Get transaction
    const tx = await this.txindex.getTx(txHash);
    // Get output hash to check
    const vaultHash = tx.outputs[txIndex].getHash();
