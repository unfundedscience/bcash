...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    // Cowperthwaite is now enabled
    if (mtp >= this.network.block.cowperthwaiteActivationTime) {
      state.cowperthwaite = true;
    }

    // Lee Kuan Yew is now enabled
    if (mtp >= this.network.block.leekuanyewActivationTime) {
      state.leekuanyew = true;
    }

    // Augusto is now enabled
    if (mtp >= this.network.block.augustoActivationTime) {
      state.augusto = true;
    }

    // Schumpeter is now enabled
    if (mtp >= this.network.block.schumpeterActivationTime) {
      state.schumpeter = true;
    }

    // Shibusawa is now enabled
    if (mtp >= this.network.block.shibusawaActivationTime) {
      state.shibusawa = true;
      state.flags |= Script.flags.VERIFY_63BITINT;
    }
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasCowperthwaite() && state.hasCowperthwaite())
      this.logger.warning('Cowperthwaite has been activated.');

    if (!this.state.hasLeekuanyew() && state.hasLeekuanyew())
      this.logger.warning('Lee Kuan Yew has been activated.');

    if (!this.state.hasAugusto() && state.hasAugusto())
      this.logger.warning('Augusto has been activated.');

    if (!this.state.hasSchumpeter() && state.hasSchumpeter())
      this.logger.warning('Schumpeter has been activated.');

    if (!this.state.hasShibusawa() && state.hasShibusawa())
      this.logger.warning('Shibusawa has been activated.');
...
class DeploymentState {
  ...
  constructor() {
    ...
    this.cowperthwaite = false;
    this.leekuanyew = false;
    this.augusto = false;
    this.schumpeter = false;
    this.shibusawa = false;
  }
  ...
  /**
   * Test whether Cowperthwaite update is active.
   * @returns {Boolean}
   */

  hasCowperthwaite() {
    return this.cowperthwaite;
  }

  /**
   * Test whether Lee Kuan Yew update is active.
   * @returns {Boolean}
   */

  hasLeekuanyew() {
    return this.leekuanyew;
  }

  /**
   * Test whether Augusto update is active.
   * @returns {Boolean}
   */
  ...
  /**
   * Test whether Schumpeter update is active.
   * @returns {Boolean}
   */

  hasSchumpeter() {
    return this.schumpeter;
  }

  /**
   * Test whether Shibusawa update is active.
   * @returns {Boolean}
   */

  hasShibusawa() {
    return this.shibusawa;
  }
